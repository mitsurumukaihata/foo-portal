...
<\/script>
    ...
    if (url.pathname === '/d1/update-product-master' && request.method === 'POST' && env.DB) {
      try {
        ...
        const { id } = body2;
        if (!id) return new Response(JSON.stringify({ error: 'id required' }), { status: 400, headers: { ...cors, "Content-Type": "application/json" } });
        const ALLOWED = ['タイヤ銘柄','メーカー','ブランド','タイヤサイズ','単価','A表定価','メモ','車種カテゴリ','廃番','商品分類'];
    ...
    if (url.pathname === '/d1/update-vehicle' && request.method === 'POST' && env.DB) {
      try {
        ...
        const ALLOWED_FIELDS = ['車番','車種','仕様','前輪サイズ','後輪サイズ','前輪パターン','後輪パターン','本数','カテゴリ','メモ','管理番号','車軸配置','顧客ID','サカイ振分確認済','大型仕様確認済','エンドユーザー'];
        const sets = [];
        const params = [];
