// 指定月内の指定伝票番号リストについて、明細から税抜合計を再計算
// 使い方: node _recalc-totals-month.mjs --year 2025 --month 8 --slips 00002167
...
if (!YEAR || !MONTH || !SLIPS.length) { console.log('--year --month --slips 必須'); process.exit(1); }
...
for (const slipNo of SLIPS) {
  const target = all.find(s => {
    ...
    return memo.includes('弥生伝票' + slipNo);
  });
  if (!target) { console.log('伝票' + slipNo + ': 見つからず'); continue; }
  const details = [];
  let dcur = null;
  do {
  ...
  const oldZ = target.properties['税抜合計']?.number || 0;
  console.log(`伝票${slipNo} (${target.id.slice(0,8)}) 明細${details.length}件 税抜 ${oldZ.toLocaleString()}→${zeinuki.toLocaleString()} 消費税→${zei.toLocaleString()} 税込→${zeikomi.toLocaleString()}`);
